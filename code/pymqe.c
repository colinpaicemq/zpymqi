...
#ifdef delete
  if ( qDescP  -> ObjectString.  VSPtr       !=  0)
  {
      printHex(stdout,qDescP  -> ObjectString.VSPtr,16);
  }
  else
    printf("c:ObjectString is null\n");

  printf("c:Object Type %d\n",qDescP -> ObjectType);
  printf("c:Object Name \n");
  printHex(stdout,qDescP  -> ObjectName,48);
  if (qDescP -> ObjectString   .VSPtr != 0 )
  {
    printf("c:MQOD ObjectString CCSID:%d \n",qDescP -> ObjectString.VSCCSID);
    printHex(stdout,qDescP -> ObjectString   .VSPtr,qDescP -> ObjectString   .VSLength);
//  __a2e_l(qDescP -> ObjectString   .VSPtr, qDescP -> ObjectString   .VSLength);
// qDescP -> ObjectString.VSCCSID  = 0;
//  printHex(stdout,qDescP -> ObjectString   .VSPtr,qDescP -> ObjectString   .VSLength);
  }
  else
    printf("c:ObjectString  is null \n");
...
//printf("c:SubLevel A %d\n", subDescP -> SubLevel);
  ...
  DEBUGIF(xSUB,compCode)
   {
   #ifdef delete
      printf("c:1367 offset ResObjectString %d\n",offsetof(MQSD,ResObjectString));
      printf("c:1367 subDescP->ResObjectString.VSBufSize   %d\n",subDescP->ResObjectString.VSBufSize);
      printf("c:1367 subDescP->ResObjectString.CCSID       %d\n",subDescP->ResObjectString.VSCCSID  );
      printf("c:1367 subDescP->ResObjectString.VSLength    %d\n",subDescP->ResObjectString.VSLength);
      printf("c:1367 subDescP->ResObjectString.VSOffset    %d\n",subDescP->ResObjectString.VSOffset);
      printf("c:1367 subDescP->ResObjectString.VSPtr       %d\n",subDescP->ResObjectString.VSPtr   );
  #endif
...
//    printf("c:MQSUB ResObjectString\n",subDescP->ResObjectString.VSLength,subDescP->ResObjectString .VSPtr);
...
//    printf("c:MQSUB Sub options %8.8x\n",subDescP-> Options);
      int i;
      int j = 1;  // used to and with options to check flags
      for (i = 0; i < 32 ;i ++)
      {
  ...
  DEBUGIF(xSETMP,comp_code)
    {
      ...
      printf("c:MQSETMP1 type %s vl %d\n", MQTYPE_STR(property_type),value_length);
...
//rv = Py_BuildValue("("PSY")", getPCFValue(which,value));
  ...
  char *  prc = getPCFValue(which,value,&pWhat,&pValue,&pPValue);
